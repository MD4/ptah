...
describe("GET /show", () => {
  ...
  it("returns 500 on service error", async () => {
    (showService.handleShowList as jest.Mock).mockRejectedValue(new Error("disk error"));
    const res = await supertest(makeApp()).get("/show");
...
describe("POST /show", () => {
  it("returns 201 on create", async () => {
    ...
    const res = await supertest(makeApp()).post("/show").send({ name: "test-show" });
  ...
  it("returns 400 on invalid body (name with space)", async () => {
    const res = await supertest(makeApp()).post("/show").send({ name: "bad name" });
    expect(res.status).toBe(400);
  });

  it("returns 500 on service error", async () => {
    (showService.handleShowCreate as jest.Mock).mockRejectedValue(new Error("write error"));
    const res = await supertest(makeApp()).post("/show").send({ name: "test-show" });
    expect(res.status).toBe(500);
  });
});
...
describe("GET /show/:name", () => {
  ...
  it("returns 500 when show not found", async () => {
    (showService.handleShowGet as jest.Mock).mockRejectedValue(new Error("not found"));
...
describe("PUT /show/:name", () => {
  it("returns 200 with updated show", async () => {
    ...
    const res = await supertest(makeApp()).put("/show/test-show").send(validShow);
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ name: "test-show" });
  });

  it("returns 400 with invalid show body", async () => {
    const res = await supertest(makeApp()).put("/show/test-show").send({ bad: true });
    expect(res.status).toBe(400);
  });

  it("returns 400 with invalid name param", async () => {
    const res = await supertest(makeApp()).put("/show/bad name!").send(validShow);
    expect(res.status).toBe(400);
  });

  it("returns 500 on service error", async () => {
    (showService.handleShowSave as jest.Mock).mockRejectedValue(new Error("write error"));
    const res = await supertest(makeApp()).put("/show/test-show").send(validShow);
    expect(res.status).toBe(500);
  });
});
...
describe("DELETE /show/:name", () => {
  ...
  it("returns 500 on service error", async () => {
    (showService.handleShowDelete as jest.Mock).mockRejectedValue(new Error("delete error"));
    const res = await supertest(makeApp()).delete("/show/test-show");
    expect(res.status).toBe(500);
  });
});
...
describe("GET /program", () => {
  it("returns 200 with list of program names", async () => {
    (programService.handleProgramList as jest.Mock).mockResolvedValue(["test-prog"]);
  ...
  it("returns 500 on service error", async () => {
    (programService.handleProgramList as jest.Mock).mockRejectedValue(new Error("disk error"));
    const res = await supertest(makeApp()).get("/program");
    expect(res.status).toBe(500);
  });
});
...
describe("POST /program", () => {
  it("returns 201 on create", async () => {
    (programService.handleProgramCreate as jest.Mock).mockResolvedValue(validProgram);
    const res = await supertest(makeApp()).post("/program").send({ name: "test-prog" });
    expect(res.status).toBe(201);
...
describe("GET /program/:name", () => {
  it("returns 200 with program data", async () => {
    (programService.handleProgramGet as jest.Mock).mockResolvedValue(validProgram);
...
describe("PUT /program/:name", () => {
  it("returns 200 with saved program", async () => {
    (programService.handleProgramSave as jest.Mock).mockResolvedValue(validProgram);
    const res = await supertest(makeApp()).put("/program/test-prog").send(validProgram);
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ name: "test-prog" });
  });

  it("returns 400 with invalid program body", async () => {
    const res = await supertest(makeApp()).put("/program/test-prog").send({ bad: true });
    expect(res.status).toBe(400);
  });

  it("returns 500 on service error", async () => {
    (programService.handleProgramSave as jest.Mock).mockRejectedValue(new Error("write error"));
    const res = await supertest(makeApp()).put("/program/test-prog").send(validProgram);
    expect(res.status).toBe(500);
  });
});
...
describe("DELETE /program/:name", () => {
  it("returns 204 on delete", async () => {
    (programService.handleProgramDelete as jest.Mock).mockResolvedValue(undefined);
...
describe("GET /settings", () => {
  it("returns 200 with settings", async () => {
    (settingsService.handleSettingsGet as jest.Mock).mockResolvedValue(validSettings);
  ...
  it("returns 500 on service error", async () => {
    (settingsService.handleSettingsGet as jest.Mock).mockRejectedValue(new Error("error"));
    const res = await supertest(makeApp()).get("/settings");
    expect(res.status).toBe(500);
  });
});
...
describe("PUT /settings", () => {
  it("returns 200 with updated settings", async () => {
    (settingsService.handleSettingsPut as jest.Mock).mockResolvedValue(validSettings);
  ...
  it("returns 500 on service error", async () => {
    (settingsService.handleSettingsPut as jest.Mock).mockRejectedValue(new Error("error"));
    const res = await supertest(makeApp()).put("/settings").send(validSettings);
    expect(res.status).toBe(500);
  });
});
